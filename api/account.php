<?php
// api/account.php - API xử lý dữ liệu tài khoản khách hàng
session_start();
...
function handleUpdateUserProfile($conn) {
    try {
        ...
        if (!empty($phone) && !preg_match('/^(0[3|5|7|8|9])[0-9]{8}$/', $phone)) {
            echo json_encode(['success' => false, 'message' => 'Số điện thoại không hợp lệ. Ví dụ: 0912345678']);
            return;
        }
        ...
        if (!empty($birthday)) {
            ...
            if ($age < 18) {
                echo json_encode(['success' => false, 'message' => 'Bạn phải từ 18 tuổi trở lên']);
                return;
            }
        ...
        if (empty($birthday)) {
            $stmt->bind_param("sssss", $fullName, $phone, null, $address, $userId);
        } else {
            $stmt->bind_param("sssss", $fullName, $phone, $birthday, $address, $userId);
        }
        ...
        if ($stmt->affected_rows > 0) {
            ...
            // Format ngày sinh để trả về
            $formattedBirthday = null;
            if ($updatedUser['ngay_sinh']) {
                $formattedBirthday = date('Y-m-d', strtotime($updatedUser['ngay_sinh']));
            }
        ...
        } elseif (strpos($e->getMessage(), 'chk_phone_format') !== false) {
            echo json_encode(['success' => false, 'message' => 'Số điện thoại không hợp lệ']);
        } elseif (strpos($e->getMessage(), 'chk_birthday_age') !== false) {
            echo json_encode(['success' => false, 'message' => 'Ngày sinh không hợp lệ (phải từ 18-100 tuổi)']);
        } else {
...
function handleUpdateAvatar($conn) {
    try {
        ...
        // Kiểm tra nếu là đặt avatar mặc định
